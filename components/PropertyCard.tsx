  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden shrink-0">
        ...
        {!readOnly && (
           <div className="absolute inset-x-0 bottom-0 p-3 translate-y-full group-hover:translate-y-0 transition-transform duration-300 bg-gradient-to-t from-slate-900/95 via-slate-900/40 to-transparent flex justify-end gap-2">
              <button 
                onClick={(e) => { e.stopPropagation(); onEdit(property); }}
                className="p-2 bg-white/10 hover:bg-blue-600 text-white rounded-xl backdrop-blur-md transition-all border border-white/10"
              >
                <Edit3 size={16} />
              </button>
              <button 
                onClick={(e) => { e.stopPropagation(); onDelete(property.id); }}
                className="p-2 bg-white/10 hover:bg-rose-600 text-white rounded-xl backdrop-blur-md transition-all border border-white/10"
              >
                <Trash2 size={16} />
              </button>
           </div>
      ...
      <div className="p-6 flex-1 flex flex-col">
        ...
        <div className="flex items-center justify-between mb-5 py-4 border-y border-slate-50 dark:border-slate-800/50">
           <div>
              <p className="text-[10px] text-slate-400 uppercase font-black tracking-widest mb-0.5">Starting From</p>
           ...
           <div className="text-right">
              <div className="flex items-center justify-end gap-1.5 text-[11px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest bg-slate-50 dark:bg-slate-800/50 px-3 py-1.5 rounded-xl">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-slate-50/50 dark:bg-slate-800/30 p-3 rounded-2xl border border-slate-50 dark:border-slate-800">
            <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Land Area</span>
            <span className="text-xs font-black text-slate-800 dark:text-slate-200">{property.totalProjectSize}</span>
          </div>
          <div className="bg-slate-50/50 dark:bg-slate-800/30 p-3 rounded-2xl border border-slate-50 dark:border-slate-800">
            <span className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Timeline</span>
