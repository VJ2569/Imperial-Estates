...
const CallHistory: React.FC = () => {
  ...
  const renderLeadStatus = (status: Lead['status']) => {
    switch (status) {
      case 'new':
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-emerald-100 text-emerald-700 dark:bg-emerald-900/40 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800"><TrendingUp size={12} /> New</span>;
      case 'contacted':
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-400 border border-blue-200 dark:border-blue-800"><Mail size={12} /> Contacted</span>;
      default:
        return <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border border-slate-200 dark:border-slate-700"><CheckCircle2 size={12} /> Closed</span>;
    }
  };
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto w-full">
          ...
          {view === 'voice' && agents.length > 1 && (
            <div className="relative">
              ...
              <select
                ...
                className="pl-9 pr-8 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-700 dark:text-gray-200 outline-none font-bold appearance-none min-w-[160px]"
              >
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[32px] shadow-sm border border-gray-200 dark:border-slate-800 overflow-hidden">
        <div className="overflow-x-auto">
                  ...
                  <tr key={call.call_id} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors group">
                    ...
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {call.recording_url && (
                          <a href={call.recording_url} target="_blank" rel="noreferrer" className="p-2 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg">
                            <Play size={18} />
                          </a>
                        )}
                        {call.transcript && (
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800">
                {leads.map((lead) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-6 font-black text-blue-600 dark:text-blue-400 text-[10px] uppercase tracking-widest">
                      {lead.project_interested || 'General'}
                    </td>
                    <td className="px-8 py-6 font-bold text-slate-800 dark:text-slate-200 text-sm">
                      ₹ {lead.budget || 'TBD'}
                    </td>
          ...
          {((view === 'voice' && filteredCalls.length === 0) || (view === 'leads' && leads.length === 0)) && !loading && (
             <div className="p-20 text-center">
                ...
                <p className="text-slate-500 text-sm max-w-xs mx-auto">Call logs and forms will sync here from n8n automatically.</p>
             </div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-md flex items-center justify-center p-4 z-[100]" onClick={() => setSelectedTranscript(null)}>
          <div className="bg-white dark:bg-slate-900 rounded-[40px] max-w-2xl w-full p-8 shadow-3xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
             <div className="flex justify-between items-center mb-6 pb-4 border-b border-gray-100 dark:border-slate-800">
                ...
                <button onClick={() => setSelectedTranscript(null)} className="p-2 hover:bg-slate-100 rounded-full text-slate-500"><X size={20} /></button>
             </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[40px] max-w-3xl w-full p-10 shadow-3xl animate-in zoom-in-95" onClick={e => e.stopPropagation()}>
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="flex flex-col">
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Requirements & Details</p>
