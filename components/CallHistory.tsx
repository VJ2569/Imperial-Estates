...
const CallHistory: React.FC = () => {
  ...
  // Fixed whitelist for string stability
  ...
  const loadData = useCallback(async (isSilent = false) => {
    ...
    try {
      ...
      const filtered = result.filter(item => {
        const id = item._uid || item.id || item.call_id;
  ...
  const handleDeleteRecord = (id: string, e: React.MouseEvent) => {
    e.stopPropagation();
    if (window.confirm("Permanently remove this record from local view?")) {
      markIdAsDeleted(id);
  ...
  const renderCellValue = (key: string, value: any) => {
    ...
    if (key === "call_status") {
      const isComp = value === "completed";
      const baseClass = "px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest";
      const themeClass = isComp ? "bg-emerald-100 text-emerald-700" : "bg-blue-100 text-blue-700";
      return <span className={baseClass + " " + themeClass}>{value}</span>;
    }
                    ...
                    <tr key={uid} onClick={() => setSelectedRecord(item)} className="hover:bg-blue-50/20 dark:hover:bg-blue-900/10 transition-colors cursor-pointer group h-20">
                      ...
                      <td className="px-10 py-2 text-right">
                         <div className="inline-flex items-center gap-3">
                            <div className="p-3 bg-slate-50 dark:bg-slate-800 group-hover:bg-blue-600 group-hover:text-white rounded-xl transition-all shadow-sm">
                               <Maximize2 size={16} />
                            </div>
              ...
              {!loading ? (
                <>
                  ...
                  <p className="text-slate-500 text-sm max-w-sm mx-auto font-medium">
                    No records found for {activeTab === 'voice' ? "Retell API" : "Google Hub"}.
                  </p>
                </>
              ) : (
                   ...
                   <div>
                      ...
                      <div className="flex items-center gap-4 mt-3">
                         <span className="px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-widest bg-blue-100 text-blue-700">
                           {activeTab === 'voice' ? "Retell Voice" : "Sheet Snapshot"}
                         </span>
